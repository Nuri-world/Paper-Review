...
Step 1. Normal Diffusion (법선 확산):경계 $\Omega$에서 얻은 초기 법선 $N$을 도메인 전체로 아주 짧은 시간($t=h^2$) 동안 확산시킵니다.벡터 열 방정식 $(M + tL^{\nabla})X_t = X_0$을 해결하여 표면의 방향 정보를 전파합니다.
Step 2. Gradient Approximation (그래디언트 근사):확산된 벡터 $X_t$를 단위 벡터화($Y_t = \frac{X_t}{\|X_t\|}$)하여, SDF의 그래디언트와 일치하는 단위 벡터장을 얻습니다.
Step 3. Distance Recovery (거리 복원):복원된 벡터장 $Y_t$의 발산(Divergence)을 소스 항으로 하는 포아송 방정식 $\Delta \phi = \nabla \cdot Y_t$을 풀어 최종적인 부호 거리 함수 $\phi$를 복원합니다.
...
My Thoughts & Connection연구 적합성: 본 논문은 제가 연구 중인 볼륨 변형 시스템에서 입력 모델의 결함을 보정하고 안정적인 SDF 필드를 생성하는 데 매우 핵심적인 이론적 토대를 제공합니다.인사이트: 단순히 거리를 계산하는 것을 넘어 법선(Normal)을 먼저 확산시킨 뒤 거리를 역추적하는 접근 방식은 방향성 오류(Orientation error)를 원천적으로 방지하는 탁월한 방법이라고 생각합니다.
